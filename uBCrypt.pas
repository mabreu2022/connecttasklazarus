...
function CryptRaw(const key, salt: TBytes; cost: Integer): TBytes;
var
...
function FormatPasswordHashForBsd(const salt, hash: TBytes; cost: Integer; const prefix: string): ansistring;
var
...
begin
  ...
  Result := Format('%s%02d$%s%s', [prefix, cost, saltString, hashString]);
end;

function HashPassword(const Str: string; const salt: ansistring; cost: Integer; const prefix: string): ansistring;
var
  password: ansistring;
...
begin
  ...
  Hash := CryptRaw(key, saltBytes, cost);
  Result := FormatPasswordHashForBsd(saltBytes, Hash, cost, prefix);
end;
...
begin
  ...
  RegexObj.Expression := '^(\$2[aby]\$)([0-9]{2})\$([\./0-9A-Za-z]{22})';
  if RegexObj.Exec(Hash) then
  begin
    PrefixStr := RegexObj.Match[1];
    CostStr := RegexObj.Match[2];
    CostVal := StrToIntDef(CostStr, 10);
    Result := HashPassword(Str, RegexObj.Match[3], CostVal, PrefixStr) = Hash;
  end
  else
  begin
    Result := False;
